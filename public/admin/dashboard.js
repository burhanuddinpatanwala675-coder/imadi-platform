...
function renderInquiries() {
    ...
    $('#inquiry-list').innerHTML = items.length ? items.map((item) => {
        const context = [
            item.projectType ? `Project: ${escapeHtml(item.projectType)}` : null,
            item.budgetRange ? `Budget: ${escapeHtml(item.budgetRange)}` : null,
            item.website ? `Website: ${escapeHtml(item.website)}` : null,
            item.toolsUsed ? `Current tools: ${escapeHtml(item.toolsUsed)}` : null,
            item.timeline ? `Timeline: ${escapeHtml(item.timeline)}` : null,
        ].filter(Boolean).join(' · ');
        return `<article class="inquiry-card"><div><h3>${escapeHtml(item.name)}</h3><p class="inquiry-meta">${escapeHtml(item.workEmail)} · ${escapeHtml(item.companyName)}</p>${statusBadge(item.status)}</div>${context ? `<p class="inquiry-meta">${context}</p>` : ''}<p>${escapeHtml(item.message)}</p><label>Status<select class="inquiry-status" data-id="${item.id}">${['new','contacted','qualified','proposal_sent','won','lost','archived'].map((status) => `<option value="${status}" ${item.status === status ? 'selected' : ''}>${status.replace(/_/g, ' ')}</option>`).join('')}</select></label><label>Assign to<select class="inquiry-assignee" data-id="${item.id}"><option value="">Unassigned</option>${state.users.map((user) => `<option value="${user.id}" ${item.assignedToId === user.id ? 'selected' : ''}>${escapeHtml(user.fullName)} (${escapeHtml(user.role)})</option>`).join('')}</select></label><label>Internal notes<textarea class="inquiry-notes" data-id="${item.id}">${escapeHtml(item.notes || '')}</textarea></label><div class="inquiry-actions"><button class="primary-button save-inquiry" data-id="${item.id}">Save changes</button><button class="text-button danger-button delete-inquiry" data-id="${item.id}">Delete</button></div></article>`;
    }).join('') : '<p class="empty">No inquiries match this filter.</p>';
}
...
function renderSettings() { if (!state.settings) return; $('#setting-company').value = state.settings.companyName || ''; $('#setting-email').value = state.settings.primaryEmail || ''; $('#setting-phone').value = state.settings.phoneNumber || ''; $('#setting-address').value = state.settings.address || ''; $('#setting-analytics').value = state.settings.analyticsId || ''; $('#setting-notifications').value = (state.settings.notificationEmails || []).join(', '); $('#setting-linkedin').value = state.settings.socialLinks?.linkedin || ''; $('#setting-x').value = state.settings.socialLinks?.x || ''; $('#setting-linkedin-founder').value = state.settings.socialLinks?.linkedinFounder || ''; $('#setting-linkedin-cofounder').value = state.settings.socialLinks?.linkedinCofounder || ''; $('#setting-portfolio-founder').value = state.settings.socialLinks?.portfolioFounder || ''; $('#setting-portfolio-cofounder').value = state.settings.socialLinks?.portfolioCofounder || ''; $('#setting-instagram').value = state.settings.socialLinks?.instagram || ''; $('#setting-facebook').value = state.settings.socialLinks?.facebook || ''; $('#setting-tiktok').value = state.settings.socialLinks?.tiktok || ''; $('#setting-youtube').value = state.settings.socialLinks?.youtube || ''; $('#setting-maflow-url').value = state.settings.socialLinks?.maflowUrl || ''; $('#setting-cfc-url').value = state.settings.socialLinks?.cfcIndexUrl || ''; $('#setting-hercommerce-url').value = state.settings.socialLinks?.hercommerceUrl || ''; $('#setting-alico-url').value = state.settings.socialLinks?.alicoUrl || ''; $('#setting-erp-url').value = state.settings.socialLinks?.erpUrl || ''; $('#setting-renaz-url').value = state.settings.socialLinks?.renazUrl || ''; }
...
function openCase(item = null) { state.editingCaseId = item?.id || null; $('#case-editor').hidden = false; $('#case-editor-title').textContent = item ? 'Edit case study' : 'New case study'; $('#case-title').value = item?.title || ''; $('#case-slug').value = item?.slug || ''; $('#case-client').value = item?.clientName || ''; $('#case-industry').value = item?.industry || ''; $('#case-project-type').value = item?.projectType || ''; $('#case-featured').checked = Boolean(item?.featured); $('#case-summary').value = item?.summary || ''; $('#case-status').value = item?.status || 'draft'; $('#case-date').value = item?.publishedAt ? item.publishedAt.slice(0, 10) : ''; $('#case-cover').value = item?.coverImageUrl || ''; $('#case-gallery').value = (item?.galleryImages || []).join('\n'); $('#case-challenge').value = item?.challenge || ''; $('#case-opportunity').value = item?.opportunity || ''; $('#case-solution').value = item?.solution || ''; $('#case-how-it-works').value = (item?.howItWorks || []).join('\n'); $('#case-process-improvements').value = (item?.processImprovements || []).join('\n'); $('#case-key-features').value = (item?.keyFeatures || []).join('\n'); $('#case-before').value = (item?.beforeState || []).join('\n'); $('#case-after').value = (item?.afterState || []).join('\n'); $('#case-outcomes').value = item?.outcomes || ''; $('#case-technologies').value = (item?.technologies || []).join(', '); $('#case-impact').value = item?.impactHeadline || ''; $('#case-scale-metrics').value = (item?.scaleMetrics || []).join('\n'); $('#case-impact-metrics').value = (item?.impactMetrics || []).join('\n'); $('#case-whats-next').value = item?.whatsNext || ''; $('#case-seo-title').value = item?.seoTitle || ''; $('#case-seo-description').value = item?.seoDescription || ''; }
...
function openPost(post = null) {
    ...
    $('#post-date').value = post?.publishedAt ? post.publishedAt.slice(0, 10) : '';
    ...
    $('#post-seo-title').value = post?.seoTitle || '';
    $('#post-seo-description').value = post?.seoDescription || '';
    $('#post-title').focus();
}
...
document.addEventListener('click', async (event) => {
    ...
    try {
        ...
        if (button.classList.contains('delete-inquiry')) { if (!confirm('Delete this inquiry permanently?')) return; await api(`/api/admin/inquiries/${button.dataset.id}`, { method: 'DELETE' }); await loadAll(); return setStatus('Inquiry deleted.', 'success'); }
...
$('#post-form').addEventListener('submit', async (event) => { event.preventDefault(); try { let coverImageUrl = $('#post-cover').value.trim() || null; const file = $('#post-image-file').files[0]; if (file) { if (file.size > 5 * 1024 * 1024) throw new Error('Image must be 5 MB or smaller.'); setStatus('Uploading image…'); coverImageUrl = (await api('/api/admin/media', { method: 'POST', body: JSON.stringify({ filename: file.name, dataUrl: await asDataUrl(file) }) })).data.url; } const body = { title: $('#post-title').value.trim(), slug: $('#post-slug').value.trim(), category: $('#post-category').value.trim(), status: $('#post-status').value, publishedAt: $('#post-date').value || undefined, tags: $('#post-tags').value.split(',').map((tag) => tag.trim()).filter(Boolean), coverImageUrl, excerpt: $('#post-excerpt').value.trim(), content: $('#post-content').value.trim(), seoTitle: $('#post-seo-title').value.trim() || null, seoDescription: $('#post-seo-description').value.trim() || null }; await api(state.editingPostId ? `/api/admin/blog/${state.editingPostId}` : '/api/admin/blog', { method: state.editingPostId ? 'PATCH' : 'POST', body: JSON.stringify(body) }); $('#post-editor').hidden = true; await loadAll(); setStatus(`Post ${state.editingPostId ? 'updated' : 'created'}.`, 'success'); } catch (error) { setStatus(error.message, 'error'); } });
$('#case-form').addEventListener('submit', async (event) => { event.preventDefault(); try { let coverImageUrl = $('#case-cover').value.trim() || null; const file = $('#case-image-file').files[0]; if (file) { if (file.size > 5 * 1024 * 1024) throw new Error('Image must be 5 MB or smaller.'); setStatus('Uploading image…'); coverImageUrl = (await api('/api/admin/media', { method: 'POST', body: JSON.stringify({ filename: file.name, dataUrl: await asDataUrl(file) }) })).data.url; } const galleryImages = $('#case-gallery').value.split('\n').map((url) => url.trim()).filter(Boolean); const body = { title: $('#case-title').value.trim(), slug: $('#case-slug').value.trim(), clientName: $('#case-client').value.trim() || null, industry: $('#case-industry').value.trim(), projectType: $('#case-project-type').value || null, featured: $('#case-featured').checked, summary: $('#case-summary').value.trim() || null, status: $('#case-status').value, publishedAt: $('#case-date').value || undefined, coverImageUrl, galleryImages, challenge: $('#case-challenge').value.trim(), opportunity: $('#case-opportunity').value.trim() || null, solution: $('#case-solution').value.trim(), howItWorks: $('#case-how-it-works').value.split('\n').map((step) => step.trim()).filter(Boolean), processImprovements: $('#case-process-improvements').value.split('\n').map((line) => line.trim()).filter(Boolean), keyFeatures: $('#case-key-features').value.split('\n').map((feature) => feature.trim()).filter(Boolean), beforeState: $('#case-before').value.split('\n').map((line) => line.trim()).filter(Boolean), afterState: $('#case-after').value.split('\n').map((line) => line.trim()).filter(Boolean), outcomes: $('#case-outcomes').value.trim(), technologies: $('#case-technologies').value.split(',').map((tech) => tech.trim()).filter(Boolean), impactHeadline: $('#case-impact').value.trim() || null, scaleMetrics: $('#case-scale-metrics').value.split('\n').map((line) => line.trim()).filter(Boolean), impactMetrics: $('#case-impact-metrics').value.split('\n').map((line) => line.trim()).filter(Boolean), whatsNext: $('#case-whats-next').value.trim() || null, seoTitle: $('#case-seo-title').value.trim() || null, seoDescription: $('#case-seo-description').value.trim() || null }; await api(state.editingCaseId ? `/api/admin/case-studies/${state.editingCaseId}` : '/api/admin/case-studies', { method: state.editingCaseId ? 'PATCH' : 'POST', body: JSON.stringify(body) }); $('#case-editor').hidden = true; await loadAll(); setStatus(`Case study ${state.editingCaseId ? 'updated' : 'created'}.`, 'success'); } catch (error) { setStatus(error.message, 'error'); } });
$('#settings-form').addEventListener('submit', async (event) => { event.preventDefault(); try { await api('/api/admin/settings', { method: 'PATCH', body: JSON.stringify({ companyName: $('#setting-company').value.trim(), primaryEmail: $('#setting-email').value.trim(), phoneNumber: $('#setting-phone').value.trim() || null, address: $('#setting-address').value.trim() || null, analyticsId: $('#setting-analytics').value.trim() || null, notificationEmails: $('#setting-notifications').value.split(',').map((item) => item.trim()).filter(Boolean), socialLinks: { linkedin: $('#setting-linkedin').value.trim(), x: $('#setting-x').value.trim(), linkedinFounder: $('#setting-linkedin-founder').value.trim(), linkedinCofounder: $('#setting-linkedin-cofounder').value.trim(), portfolioFounder: $('#setting-portfolio-founder').value.trim(), portfolioCofounder: $('#setting-portfolio-cofounder').value.trim(), instagram: $('#setting-instagram').value.trim(), facebook: $('#setting-facebook').value.trim(), tiktok: $('#setting-tiktok').value.trim(), youtube: $('#setting-youtube').value.trim(), maflowUrl: $('#setting-maflow-url').value.trim(), cfcIndexUrl: $('#setting-cfc-url').value.trim(), hercommerceUrl: $('#setting-hercommerce-url').value.trim(), alicoUrl: $('#setting-alico-url').value.trim(), erpUrl: $('#setting-erp-url').value.trim(), renazUrl: $('#setting-renaz-url').value.trim() } }) }); await loadAll(); setStatus('Site settings saved.', 'success'); } catch (error) { setStatus(error.message, 'error'); } });
